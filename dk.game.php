 ...
 *
 */
include "misc/_bga_ide_helper.php";
...
class dk extends Table
{
  ...
  protected function setupNewGame($players, $options = array())
  {
    ...
    self::setGameStateInitialValue('chosen', 0);
  ...
  protected function getAllDatas()
  {
    ...
    $result['cardSorting'] = self::getCurrentCardSorting();

    $result['hand'] = self::getHand($current_player_id);
    ...
    if ($suit != TRUMP) {
      return 0 < self::getUniqueValueFromDB(
        "SELECT
          COUNT(`card`.`card_id`)
        FROM
          `card` WHERE `card`.`card_location_arg` = '$player' AND `card`.`card_location` = 'hand' AND `card`.`card_trumpN` = '0' AND `card`.`card_type` = '$suit';"
      );
    } else {
      return 0 < self::getUniqueValueFromDB(
        "SELECT COUNT(`card`.`card_id`) FROM `card` WHERE
          `card`.`card_location_arg` = '$player' AND `card`.`card_location` = 'hand' AND `card`.`card_trumpN` > '0';"
      );
    }
  }
  ...
  {
    return self::getUniqueValueFromDB("SELECT `card_trumpN` FROM `card` WHERE `card`.`card_id` = '${$card['id']}'");
  }

  public function getTrumpQ($card)
  {
    return self::getUniqueValueFromDB("SELECT `card_trumpQ` FROM `card` WHERE `card`.`card_id` = '${$card['id']}'");
  }
  ...
  public function reservation()
  {
    ...
    self::notifyAllPlayers('reservation', '${player_name} has VORBEHALT.', array(
      'player_name' => self::getActivePlayerName(),
      'player_id' => $player_id
    ));
  ...
  {
    self::checkAction('mandatorySolo');
    $player_id = self::getActivePlayerId();
    ...
    self::setGameStateValue('solo', $solo);

    self::notifyAllPlayers('solo', '${player_name} plays a mandatory ${solo_name}.', array(
      'player_name' => self::getActivePlayerName(),
      'player_id' => $player_id,
      // TODO solo
      'solo_name' => "queen solo"
    ));

    $this->gamestate->nextState(('startGame'));

    //$this->gamestate->changeActivePlayer(self::getStartPlayer());
  }
  ...
  */
  ...
  public function stNewHand()
  {
    ...
    foreach ($players as $player_id => $player) {
      ...
      self::notifyPlayer($player_id, 'newHand', '', array(
        'hand' => self::getHand($player_id),
        'cardSorting' => self::getCurrentCardSorting()
      ));
    }
  ...
  public function stPreRound()
  {
    ...
    // TODO wedding
    $wedding = 1 == self::getUniqueValueFromDB(
      "SELECT
    ...
    $tens = 7 <= self::getUniqueValueFromDB(
      "SELECT
          MAX(neunenCount)
        FROM
          (
    ...
    if (self::getGameStateValue("autoThrow") && ($neunen || $tens || $neunenFarb || $neunenKönige || $goodTrump || $trump)) {
  ...
  `card` c ON p.`player_id` = c.`card_location_arg` 
     AND c.`card_type` = 4 AND c.`card_type_arg` = $QUEEN
        SET
          p.`player_re` = '1';"
    );
    ...
    self::setGameStateValue("chosen", 4);
    $this->gamestate->nextState('playerChoose');
  }

  public function stNextReservation()
  {
    self::incGameStateValue('chosen', -1);

    if (self::getGameStateValue('chosen') == 0) {
      // End of the trick
  ...
  public function stNextPlayer()
  {
    ...
    if ($this->cards->countCardInLocation('table') == 4) {
      ...
      if (self::getGameStateValue('wedding') == 1) {
        if ($this->cards->countCardInLocation('won') <= 4 * 3) {
          if ($players[$best_card['location_arg']]['player_re'] == 0) {
            self::DbQuery(
              "UPDATE `player` SET `player_re` = '1' WHERE `player_id` = " . $best_card['location_arg']
            );
            self::setGameStateValue('wedding', 0);
            self::notifyAllPlayers('weddingComplete', clienttranslate('${player_name} married and is now RE as well.'), array(
              'player_name' => $players[$best_card['location_arg']]['player_name']
            ));
          } else if ($this->cards->countCardInLocation('won') >= 4 * 3) {
            self::setGameStateValue('wedding', 0);
            self::notifyAllPlayers('weddingComplete', clienttranslate('${player_name} was unable to find a partner. Playing Solo now.'), array(
      ...
      if ($this->cards->countCardInLocation('hand') == 0) {
        if ($best_card['type'] == $CLUB && $best_card['type_arg'] == $JACK) {
          self::setGameStateValue('charlie', $best_card['location_arg']);
          self::notifyAllPlayers('giveSpecToPlayer', clienttranslate('${player_name} takes a Charlie Miller'), array(
            'player_name' => $players[$best_card['location_arg']]['player_name'],
            'player_id' => $best_card['location_arg'],
  ...
  public function stEndHand()
  {
    ...
    $charlie_id = self::getGameStateValue('charlie');
    $re_charlie = self::getUniqueValueFromDB(
      "SELECT
          COUNT(`player_id`)
        FROM
          `player`
    ...
    $kontra_charlie = self::getUniqueValueFromDB(
      "SELECT
          COUNT(`player_id`)
        FROM
          `player`
    ...
    foreach ($re_player_ids as $key => $value) {
      self::dbIncScore($key, ($re_score > $kontra_score ? 4 - intdiv($kontra_score, 30) : 0) + $re_fox_count + $re_dk_count + $re_charlie);
    }

    foreach ($kontra_player_ids as $key => $value) {
      self::dbIncScore($key, ($re_score <= $kontra_score ? 5 - intdiv($re_score, 30) : 0) + $kontra_fox_count + $kontra_dk_count + $kontra_charlie);
    }

    if ($re_score > $kontra_score) {
      self::notifyAllPlayers('winner', clienttranslate('RE won with ${re_score} Points, ${re_charlie} times Charlie Miller, ${re_fox_count} times Fox and ${re_doppelkopf_count} times Doppelkopf vs
      ...
      self::notifyAllPlayers('winner', clienttranslate('KONTRA won with ${kontra_score} Points, ${kontra_charlie} times Charlie Miller, ${kontra_fox_count} times Fox and ${kontra_doppelkopf_count} times Doppelkopf, vs
